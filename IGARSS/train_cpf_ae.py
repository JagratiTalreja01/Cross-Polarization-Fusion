...
def main():
    ...
    for epoch in range(1, 51):
        ...
        model.train()
        train_loss = 0.0
        for x, y, valid, _ in tqdm(train_dl, desc=f"Train AE {epoch:02d}"):
        ...
        val_loss = 0.0
        with torch.no_grad():
            for x, y, valid, _ in tqdm(val_dl, desc=f"Val   AE {epoch:02d}"):
                x = x.to(device, non_blocking=True)
                y = y.to(device, non_blocking=True)
                valid = valid.to(device, non_blocking=True)
        ...
        if val_loss < best_val:
            best_val = val_loss
            torch.save(model.state_dict(), "cpf_autoencoder_deepflood.pth")
            print("✅ Saved best CPF-AE model: cpf_autoencoder_deepflood.pth")
