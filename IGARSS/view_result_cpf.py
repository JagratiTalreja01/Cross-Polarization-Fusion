...
def cpf_composite(cpf_vv: np.ndarray, cpf_vh: np.ndarray) -> np.ndarray:
    """
    RGB visualization for CPF(VV,VH):
      R = CPF-VV (norm)
      G = CPF-VH (norm)
      B = mean(VV,VH) (norm)
    """
...
def save_rich_panel(
    ...
    # make raw vv/vh match prob shape
    if vv_raw.shape != (H, W):
...
def main():
    # Examples:
    #   python view_results_cpf_unet_richpanel.py
    #   python view_results_cpf_unet_richpanel.py --n 12 --thr 0.4
    #   python view_results_cpf_unet_richpanel.py --ckpt cpfunet_deepflood.pth --csv out/test.csv
    n = 8
    seed = 42
    thr = 0.5

    csv_path = OUT_DIR / "test.csv"
    ckpt = Path("cpfunet_deepflood.pth")
    ...
    if not ckpt.exists():
        raise FileNotFoundError(f"Missing {ckpt}. Train first: python train_cpf.py")
    ...
    out_vis = OUT_DIR / "vis_richpanel_unet"
    ...
    with torch.no_grad():
        for k, i in enumerate(idxs, start=1):
            ...
            prob = model(x1).squeeze().cpu().numpy().astype(np.float32)
            if prob.ndim == 3:
                prob = prob[0]
            ...
            title = f"{tile_id} | CPF-UNet | Dice={d:.3f} IoU={j:.3f} | thr={thr}"
